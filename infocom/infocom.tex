...
instead of $1,000$. The number of different orders is $7!= 5,040$ since we have $8$ routes and the solutions are invariant up to a circular permutation of the order. Therefore instead of doing $10,000$ random draws we could test every possible order in less time. However the computation time of this method would scale badly with $n$. On the other hand, remark that the success rate of PMLS is already high for $100$ random orders, which means it will work even better than the other methods when $n$ is larger and that the number of random orders drawn becomes critical.
    ...
    \begin{figure}
       ...
       \begin{center}
     \hspace{-0.5cm} \includegraphics[width = 0.4\textwidth]{stochastic.pdf}
 ...
 \section{Conclusion}
In this paper, we proposed two kinds of solution to ensure a low latency for C-RAN in fronthaul networks. First, we can find some assignments without buffering the messages in the BBUs when the routes are short, by the shortest longest policy, or when the load of the network is small, with the greedy algorithm. The experience with the FPT algorithm shown that there is some instances when the network is highly loaded in which we need to use some buffers in BBU. Thus, we developed the PMLS algorithm, that gives some assignments even with restricted parameters.

The future work is to adapt the main ideas of those algorithm to others common networks topologies, like an optical ring, or a tree.
