...
In order to allow as much antennas as possible on the ring, the idea is to use \emph{all} containers in a given position. To do so we assign the antennas to the first position which is not full. When less than $ET$ free containers remain in this position ( let us call $x$ this number of free containers), then the $x$ first containers of an antenna are assigned to this position, and the $ET - x$ last containers of this antennas are assigned to the next position. Fig.~\ref{fig:split} illustrates this manner to find an assignment with a maximal number of antennas. 

Remark that we consider only the even positions, because the BBU are set by definitions on the odd positions. This mean that a container reserved to the next position means in reality that this container is reserved $2$ positions after. Thus, a message arriving in a node from an RRH at time $m_i$ can see its reserved container at time $m_i +2$ which introduce a bit of additional latency of $2\mu$s for a part of the CRAN traffic. However, the number of antennas supported by the ring is now  $\lfloor\frac{P- RS}{ET} \times \frac{F}{2}\rfloor$, which is a considerable gain with our N-GREEN parameters ($9$ antennas instead of $5$).
