...
\subsection{Reservation, assignment}
...
\subsection{A smarter approach for N-GREEN parameters}
\label{sec:maxant}
...
\paragraph{Fulfilling positions}
In order to allow as much antennas as possible on the ring, the idea is to use \emph{all} containers in a given position. To do so we assign the antennas to the first position which is not full. When less than $ET$ free containers remain in this position ( let us call $x$ this number of free containers), then the $x$ first packets of an antenna are assigned to this position, and the $ET - x$ last packets of this antennas are assigned to the next position. Fig.~\ref{fig:split} illustrates this manner to find an assignment with a maximal number of antennas. 

Remark that we only consider the even positions, because the BBU are set by definitions on the odd positions. This mean that if a container is reserved in the next position, it is, in reality reserved $2$ positions after. Thus, a packet arriving in a node from an RRH at time $m_i$ can see its reserved container at time $m_i +2$, which introduce a bit of additional latency of $2$ unit of time for a part of the CRAN traffic. However, this cost is worth because the number of antennas supported by the ring is now  $\lfloor\frac{P- RS}{ET} \times \frac{F}{2}\rfloor$, which is a considerable gain with our N-GREEN parameters ($9$ antennas instead of $5$).
