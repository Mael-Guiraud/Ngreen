...
\begin{figure}
   ...
   \section{The star topology: minimizing latency}\label{sec:PALL}
	...
	Therefore to avoid collision, we set the waiting time  $w_1 =  2\lambda(r_{0})+\tau - (\tau + 2\lambda(r_{1})) = 2(\lambda(r_{0}) - \lambda(r_{1}))$. 
	
	We generalize this by setting $w_i = 2(\lambda(r_{0}) - \lambda(r_{i}))$ for all $i< n$.
