...
\section{Model}
  ...
  \subsection{Definitions}
   ...
   \subsection{Slotted time Model}
      ...
      A {\bf $(P,\tau)$-periodic affectation} of a routage graph $\cal R$ is a sequence  ${\cal M}=(m_0, \ldots ,m_{c-1})$ of $n$ integers that we call {\bf offsets}, with $n$ the number of routes in $\cal R$. The number $m_i$ represents the index of the first slot used in a period  by the route $r_i \in {\cal R}$ at its source.
      A $(P,\tau)$-periodic affectation must have no {\bf collision} between two routes in ${\cal R}$, that is $\forall (r_i, r_j) \in {\cal R_{\cal C}}^2, i \neq j$, % with $\tau$ the size (in number of consecutive slots) of each message that must be periodically sent on each route of ${\cal R}_{\cal C}$, 
...
\begin{proof}
      ...
      If we consider a period of $2$, for all $i \neq j$, $\lambda(v,r_i) - \lambda(v,r_j) \mod 2 = 1$ . Therefore $(0,\dots,0)$ is a $(2,1)$-periodic affectation of ${\cal R}$.
...
%       %\end{tabular}\newline
      ...
      The Routing function ${\cal R}$ associtates to each couple $(s,{\cal C}(s))$ a route called $r_s$ and to each couples $({\cal C}(s),s)$ a route called $r_{{\cal C}(s)}$.\\     
...
\section{The Star Topology}
	...
	\end{center}
      ...
      Let us call {\bf forward period} and {\bf backward period} the time windows in respectively $c_s$ and $c_t$. A message issued by the source $s_1$ at time $m_{s_1}$ will reach $c_s$ at time $m_{s_1} + t(c_s,r_{s_1})$ in the forward period, and  $c_t$ at time $m_{{\cal C}(s_1)} + t(c_t,r_{{\cal C}(s_1)})$ in the backward period.
...
%       Let us call {\bf SN} (source node) the central node connected to leaves, and {\bf SN} (sources node), the one connected to sources. The period in those nodes are respectively called {\em forward period}, and {\em backward period}.
  ...
  equal to $0$. Thus, in this case, we can consider only the forward and backward periods. The goal is now to put messages in the forward periods such that there is no collisions in the both periods.\\
  In this section, the offsets $m_{s_i}, i<n$ are in reality $[t(c_s,r_{s_i})]$, but to find the real values, there is just to change the values of the offset considering the dealy of the arcs $(s_i, c_s) , \forall i$.
    ...
    We send the messages in order from the shortest route to the longest route, on after each others in the forward period. 
...
Recursively, the second message is sent before the others, and also will be back before the following messages. 
Because the second message is sent T slots after the first message ($m_{s_2} = m_{s_1} + \tau$, it comes back after
the end of the message one: $m_{s_1}+ \tau + 2.\lambda(r_{{\cal C}(s_1)}) \le m_{s_2} + 2.\lambda(r_{{\cal C}(s_2)})$ with $\lambda(r_{{\cal C}(s_2)}) \ge \lambda(r_{{\cal C}(s_1)})$.

      \paragraph{Period}
The period given by this policy is $P = \tau * n + 2(\lambda(r_{s_{n-1}}) - \lambda(r_{s_0}))$, if routes $\{r_0,...,r_{s_{n-1}}\}$ are ordered
from the shortest to the longest. $2.\lambda(r_{s_0})$ is the first slot in which the first message is coming back in the backward period,
...
the period is.
    ...
    \begin{proof}
     ...
     $(P,\tau)$-periodic affectation. To ensure a complexity linear in $n$, it is enough to maintain a list of the free slices in the forward period and a list of the free intervals in the backward period. 
    \end{proof}
