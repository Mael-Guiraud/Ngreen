...
\begin{figure}
   ...
   We plan to generalize our study of the PALL problem to more topology,
   such as trees and cycles which are the most common ones in real networks. 
   We would like to design FPT algorithms in the number of routes for PAZL and PALL on the star topology, eventually by relaxing the contraint on the maximal process time into an constraint on the average process time, which seems much more tractable. 
   Moreoever we could allow preemption, that is the messages are allowed to be cut into pieces,
   which could certainly change the complexity of the problem. 
