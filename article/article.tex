...
\footnote{This work is partialy supported by the french  ANR project N-GREEN.}
    ...
    \hline
     ...
     \hline
      ...
      $\lambda (u_i,r)$ & The latency of the vertex $u_i$ through $r$\\
         \hline
         $\lambda (r)$ & The latency of a route $r$.\\
         \hline
         $P$ & Period\\
         \hline
...
% 	    
    ...
    \subsection{Experimental Evaluation}\label{sec:exp_PAZL}
      ...
      is at most $n = 20$, $\tau$ is equal to $2500$. Moreover the period is always larger than $n\tau$ otherwise 
      no assignment is possible and smaller than $3n\tau$ otherwise the greedy algorithm always returns a solution. 
      The code in C is available online on the webpages of the authors ~\cite{webpage}.
...
\begin{figure}
   ...
   \section{The star topology: minimizing latency}\label{sec:PALL}
    ...
    In this section, we still work on the star topology, but we consider again the  PALL problem. 
    It means that the messages can wait in the target vertices (BBU), in order to find solution that we cannot find without waiting time. We allow the process times $PT$ of the routes to be greater than twice the weight of the routes, but it should less than $T_{max}$.
	...
	A more realistic assumption would be that all weigths of the arcs $(s_i,c_s)$ are the same. It would correspond
	to a situation where all the BBUs are in the same datacenter.
	
	 \begin{theorem}
	 Let $(G,{\cal R})$ be a symmetric routed network with $n$ routes and let $P \geq n\tau$. If for all $i$ and $j$ less than $n$ $\omega(s_i,c_s) = \omega (s_j,c_s)$, then there is a $(P,\tau)$-peridoic assignment with process time $2\max_{r\in {\cal R}} \lambda(r)$ and it can be built in time $O(n\log(n))$.
	 \end{theorem}
      \begin{proof}
      
        We can simplify the weights on the arcs $(s_i,c_s)$ by setting them all to $0$. We order the routes
        in decreasing order of their length, that is when $i<j$, $\lambda(r_{i}) > \lambda(r_{j})$.
        The offset of the route $r_i$ is $i\tau$, which ensures that there are no collision on the way forward (on the arc 
        $(c_s,c_t)$) as soon as $P \geq n\tau$ which is the minimal possible period.        
	We set the waiting time of the first message to $0$, thus it reaches the vertex $c_t$ for the second time before going through the arc $(c_t,c_s)$ at time $2\lambda(r_{0})+\tau$. The process time of the first message is $PT(r_0) = 2\lambda(r_{0})$.
	The second message reaches $c_t$ for the second time at $\tau + 2\lambda(r_{1})$. 
	Therefore to avoid collision, we set the waiting time  $w_1 =  2\lambda(r_{0})+\tau - (\tau + 2\lambda(r_{1})) = 2(\lambda(r_{0}) - \lambda(r_{1}))$, which is positive since $\lambda(r_{0}) > \lambda(r_{1})$. 
	
	We generalize this by setting $w_i = 2(\lambda(r_{0}) - \lambda(r_{i}))$ which are positive for all $i< n$,
	since by definition.
	There are no collision on the way back since the message of the route $r_i$ arrives at $c_t$
	at time $(i-1)\tau + 2\lambda(r)$. We must also evaluate the process time. 
	By defintion $PT(r_i) = 2\lambda(r_i) + w_i = 2\lambda(r_i) + 2(\lambda(r_{0}) - \lambda(r_{i})) = 2\lambda(r_0)$.
	Since all process times are the same, the maximum process time is also equal to $2\lambda(r_0)$.
	
	Finally, the time to build this assignment is dominated by the sorting of the routes from the longest to the shortest and is thus in $O(n\log(n)$.
     \end{proof}
\todo{Une remarque sur le fait que la complexite depend sans doute du nombre de valeurs differentes sur les arcs et pas du nombre de routes.}
