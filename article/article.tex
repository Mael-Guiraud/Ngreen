...
%opening
\title{DRAFT : Scheduling Periodic Messages in a Network by a SDN approach}
...
The pair $(G,\cal R)$ is called a {\bf routed network} and represents our network.
The first vertex of a route models an antenna (RRH) and the last one a data-center (BBU) which processes the messages sent by the antenna. We denote by $\rho$ the bijection induced by ${\cal R}$, $\rho(s) = t$ if $s$ is the first vertex of a route $r$ and $t$ the last. 
   ...
   \subsection{Slotted time Model}
      ...
      A {\bf $(P,\tau)$-periodic assignment} of a routed network $(G,\cal R)$ is a sequence  ${\cal M}=(m_1, \ldots ,m_n)$ of $n$ integers that we call {\bf offsets}, with $n$ the number of routes in $\cal R$. The number $m_{i}$ represents the time at which a message is emitted at the first vertex of the route $r_{i}$. We will always consider that the $m_{i}$ are between $0$ and $P-1$. A $(P,\tau)$-periodic assignment must have no {\bf collision} between two routes in $\cal R$, that is $\forall (r_{i}, r_{j}) \in {\cal R}^2, i \neq j$, % with $\tau$ the size (in number of consecutive slots) of each message that must be periodically sent on each route of ${\cal R}_{\cal C}$, 
...
%       Notice that the notion of $P$-periodic assignment \textbf{is not monotone} with regard to $P$. 
      As an example of a $(2,1)$-periodic assignment, let us consider a routed network with routes $\{r_{i}\}_{i=1,\dots,n}$, such that all pairs of routes intersect at a different edge.
...
%       %\end{tabular}\newline
      ...
      we must send the answer from $\rho(s)$ back to $s$. We say that a routed network $(G, {\cal R})$ is \textbf{symmetric}
	...
	This assignment represents the following process: first a message is sent at $s$, through the route $r$, at time $m_s$.
...
We simulated a stochastic network on $1000$ instances with $8$ long routes (links between $0$ and $20000$). The results are not really different if we use short routes. The flexibility needed by an instance of the network is computed by taking the message which have the highest process time, at which we subtract the delay of the longest route. We measure, for periods from $20000$ ( network fully loaded), to $50000$ ($40 \%$ of load), the medians, the first, and the third quartiles of $1000$ results.
