...
\subsection*{An FTP algorithm}

In this section we show how every assignments without waiting time can be put in a special form.
We use that to provide an algorithm which always finds an assignment if there is some, in fixed parameter 
tractable time with parameters $n$ the number of routes. This is justified since $n$ is small in practice, 
at most $20$ and $P$ is large, $20000$ in our context.


We say that an assignment $\mathcal{M}$ is \textbf{compact} if for all forward routes $r$ but one,
there is another forward route $r'$ such that $r$ and  $r'$ ``have no gap'' or $\rho(r)$ and  $\rho(r)'$ ``have no gap''.

\begin{proposition}
Let $(G, {\cal R})$ be a star routed network. If there is a $(P,\tau)$-periodic assignment to blah,
then there is a compact $(P,\tau)$-periodic assignment.
\end{proposition}
\begin{proof}
 On fait tourner les paquets jusqu'a contact on et on repete jusqu'a epuisement. 
\end{proof}

\begin{theorem}
     Let $(G, {\cal R})$ be a star routed network with $n$ routes. There is an algorithm which finds a $(P,\tau)$-periodic assignment of $(G, {\cal R})$ with all waiting times $0$ if there is one in time $O(2^nq(|G|))$ with $q$ a polynomial.
\end{theorem}

\begin{proof}
 Brute force on the compact assignments. 
\end{proof}
