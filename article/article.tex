...
\begin{figure}
   ...
   \section{The star topology: minimizing latency}\label{sec:PALL}
    ...
    It means that the messages can wait in the target vertices (BBU), in order to find solution that we cannot find without waiting time. We allow the process times $PT$ of the routes to be greater than twice the weight of the routes, but it should not be superior to $T_{max}$.

	\subsection{Special cases}
		
		
	Often in real networks, the length of the routes are not arbitrary. 
	\todo{Reprendre les deux cas faciles, est-ce qu'il y en a d'autres ?}
