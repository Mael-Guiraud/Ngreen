...
%opening
\title{DRAFT : Periodic Scheduling Messages in a Network by a SDN (deterministic) approach}
...
star topology, where messages can collide on only one arc.
...
\begin{figure}
   ...
   \section{The star topology: minimizing latency}\label{sec:PALL}
    ...
    In this section, we still work on the start topology, but, from now, we allow the messages to wait in the target vertices (BBU),in order to give an higher degree of freedom to the scheduling. Thus, the process time  $PT$ of a message might be superior to $T_{max}$. The goal is to find a solution with waiting time such that any message will be not be buffered too much, i.e. $MT({\cal M}) \leq T_{max}$.
	...
	Another particular case is the following one:	
	 \begin{theorem}
      Instances of PALL with $\omega(c_t,t_i) = 0, \forall i \leq n$ always got an optimal solution.
      \end{theorem}
      \begin{proof}
	...
	In an optimal solution, if $r_i$ is the longest route, we want $w_i =0$, such that $PT(s_i) = 2.\lambda(r_{s_i}) \geq  2.\lambda(r_{s_j}) + w_j = PT(s_j) $, where $j$ is any route $\neq i$.
     ...
     We first suggest the following heuristic, composed of two greedy parts. First, on the forward period, for the way forward, the messages are scheduled consecutively, from the one using the longest route, to the one using the shortest route. Thus, we set the $m_{s_i}$ of the routes such that  $t(c_s,r_{s_i}) = \tau.i$, if the routes $r_{s_i}$ are ordered from the longest to the shortest.
     
     Thus, to schedule the messages on the backward period, we assign to each route it's deadline, equal to $m_{s_i} + T_{max}-t(c_s,r_{s_i})$, that is,  the latest date at which a message is able to quit the target switch without exceed $T_{max}$. 
     We then determine the eligible message. At a date $d$, if one or several messages are able to come back to $c_t$ before $d$, the eligible route is the one which have the lowest deadlines. Otherwise, if any messages are able to be back on $c_t$ before $d$, the eligible route is the one with the first message able to come back to $c_t$.
...
\STATE compute the deadlines of each routes.
   ...
   \ENDIF
	...
	We tried to look at the behavior of that policy with different kinds of sending order for the messages.
	
	We tried the five following sending orders, each one determines the order in which messages will be sent from sources. In every case the messages are consecutive.
