...
%       %\end{tabular}\newline
    ...
    The {\bf maximum process time} of the $P$-periodic affectation ${\cal M} $ is defined by $MT({\cal M})=\max\limits_{l \in L} PT(l)$. The problem we want to solve is the following. 
      ...
      \noindent {\bf Input:}  a matching $\rho$ from $L$ to $S$ two disjoint set of elements, a routage graph $\cal R$ whose set of sources is $S$ and whose set of targets is $T$, a period $P$, an integer $\tau$, an integer $T_{max}$.
