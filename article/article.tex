...
Next generations of mobile network architectures evolve toward centralized radio network architectures or C-RAN, for Cloud Radio Access Network, to reduce energy consumption costs~\cite{mobile2011c}. The main challenge for this type of architecture is the latency in the transfer process between the Remote Radio Heads (RRHs) on the field and the Baseband Units (BBUs) in the cloud. The standard requires meeting time constraints for functions like HARQ (Hybrid Automatic Repeat reQuest) that needs to be processed in 1 to 10ms depending on targeted services~\cite{bouguen2012lte}. One specificity in the C-RAN context is not only the latency constraint, but also the periodicity of the data transfer between RRH and BBU.
 ...
 The network topology is modeled by a directed weighted graph and a set of routes (paths) from source nodes (RRH) to target nodes (BBUs). The time is discretized and a unit of time corresponds to the transmission of a minimal unit of data over the network.
 The process must be periodic, that is if we look at the network at time $t$ and $t+P$ where $P$ is the period, all messages are at the same position in the networks. Moreover, we want to avoid any  buffering in internal nodes of the graph, therefore the periodic process we need to design cannot collision between packets. To ensure that we need to propose a deterministic process. 
 We have two sets of parameters that we can choose when building a periodic sending process or a \textbf{periodic assignment}, first the time at which each packet is sent by a RRH in each period and the waiting time in the BBU before the answer is sent back to the RRH. We have two major difficulties to overcome when building an periodic assignment. First the periodicity which makes many scheduling methods inapplicable, since one must ensure that a message does not collide with the other messages sent by the others BBU/RRH in the same period, but also in the previous and following periods. Second, one must minimize the latency, that is the time 
 between the emission of a message and the complete return of its answer. It means that the only buffering we are allowed in the BBU
 (the waiting time before sending back the answer) must be small, in particular when the route is long.
...
\subsection*{Article organization}
 In Sec.~\ref{sec:def} we define our model, we formalize our problems PRA (one way periodic assignment) and PALL (two ways periodic assignment) and study their basic properties.
In Sec.~\ref{sec:complexity}, we prove that the problem PRA is $\NP$-hard and cannot be approximated even for very
restricted graphs. In the next two sections, we study the problem PALL case on a star topology which often arises in practice.
Since the problem is already quite complicated, we first study a variant called PAZL in Sec.\ref{sec:PAZL}, where the waiting times
are all zero. We provide several algorithms, some of their theoretical properties and give experimental evidences that they work well
when the network is no too loaded. Finally in Sec.\ref{sec:PALL}, we propose algorithms for the general problem and exhibit one which works extremely well in our experiments even in loaded networks. In particular we show that the deterministic communication schemes we design vastly outperform the traditional stochastic multiplexing. 


\section{Model}\label{sec:def}

  \subsection{Definitions}
  
\todo{simplifier car les chemins sont toujours donnés dans l'article -> du coup le problème c'est un graphe + l'ensemble de routes
+ remplacer partout affectation par assignment -> simplifier au max et uniformiser les notations}
...
\end{tikzpicture}
  ...
  \subsection{Algorithms for PAZL}
...
\begin{figure}
  ...
  \end{algorithm}
   ...
   \ENDIF
	...
	\begin{enumerate}
	 \item Random: Generate $X$ random permutations for the routes and tries the greedy heuristic on the way back until we get an available $(P,\tau)$ periodic affectation. In the following experiments, we take $X = 100$.
...
\begin{figure}
     ...
     First, we remark that, for any sending order, the success rate is strictly better for large periods. This is due to the fact that the size of the period is a factor for failure. According to our results, the two SL sending orders are not good, in any cases. However, the LS orders seem to increase when the flexibility is large enough, but in the two regimes, they appear to be stagnating to a lower success rate than Random. As we can see the random finds near to 100\% of the solutions when the period is large enough, even when we give no flexibility. On average, it is necessary to draw 6 randoms sending order to find a solution, when the period is large, but in some particularly hard instances, it can be impossible to find a solution, even with 100 randoms sending orders. Since this algorithm is not really time expensive, it could be interesting to increase the number of sending orders generated, to decrease the probability of a failure.
    
    Insister sur l'importance de ce nombre d'instances generess, par rapport au nombre d'instances possibles
    ...
    \begin{figure}
     ...
     ici pas besoin de tester pour des periodes longues : gp est deja au max, MSP strictment meilleur que GP ?
 ...
 Ouvertures -> les autres topologies (arbres, cycles) +
 découper les messages pour rendre ça plus simple
