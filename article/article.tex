...
\author{
   ...
   Ma\"el  {\bf Guiraud}, {\em UVSQ}
      \and
   Olivier  {\bf Marc\'e}, {\em Nokia Bell Labs France}
      \and
...
The first vertex of a route models an antenna (RRH) and the last one a data-center (BBU) which processes the messages sent by the antenna. We denote by $\rho$ the bijection induced by ${\cal R}$, $\rho(s) = t$ if $s$ is the first vertex of a route $r$ and $t$ the last. 
   ...
   \subsection{Slotted time Model}
      ...
      A {\bf $(P,\tau)$-periodic assignment} of a routage graph $(G,\cal R)$ is a sequence  ${\cal M}=(m_1, \ldots ,m_n)$ of $n$ integers that we call {\bf offsets}, with $n$ the number of routes in $\cal R$. The number $m_{i}$ represents the time at which a message is emitted at the first vertex of the route $r_{i}$. We will always consider that the $m_{i}$ are between $0$ and $P-1$. A $(P,\tau)$-periodic assignment must have no {\bf collision} between two routes in $\cal R$, that is $\forall (r_{i}, r_{j}) \in {\cal R}^2, i \neq j$, % with $\tau$ the size (in number of consecutive slots) of each message that must be periodically sent on each route of ${\cal R}_{\cal C}$, 
...
%       Notice that the notion of $P$-periodic assignment \textbf{is not monotone} with regard to $P$. 
      As an example of a $(2,1)$-periodic assignment, let us consider a routage graph with routes $\{r_{i}\}_{i=1,\dots,n}$, such that all pairs of routes intersect at a different edge.
...
\end{tikzpicture}
  ...
  We propose two algorithms that always find a solution in polynomial time if the instances meets some criteria (short routes or great enough period) but which may fail in other cases and one exponential algorithm which always finds a solution if it exists.
...
\begin{figure}
   ...
   \section{The star topology: minimizing latency}\label{sec:PALL}
    ...
    In this section, we allow the messages to wait in the target vertices (BBU), to give an higher degree of freedom to the scheduling. Thus, the process time of a message $PT$ might be superior to $T_{max}$. The goal is to find a solution with waiting time such that any message will be not be buffered too much, i.e. $MT({\cal M}) \leq T_{max}$.
	...
	Another particular case is the following one:	
	 \begin{theorem}
      There is always an optimal solution when all the weights on the arcs between $c_s$ and the sources are the same.
      \end{theorem}
      \begin{proof}
      

	Since we can simplify the weight on the arcs between $c_s$ and the sources by $0$, we consider only the delay between $c_t$ and the targets. Order the routes such that $\lambda(r_{s_i}) > \lambda(r_{s_j}), i<j$, i.e. the route $0$ is the longest, and the route $n-1$ is the shortest one.
	The messages are sent in the forward period from $0$ to $n-1$, that is $m_{s_i} = \tau * i$.
	...
	Consequently, for this case, sending the message from the one using the longest route to to one using the shortest route is optimal, because the longest route has no waiting time all the other routes has the exact same process time, thus, $MT(M) = PT(s_0) = 2.\lambda(r_{s_0}) $. Moreover, the period used is exactly $n \tau$, in both the forward and backward periods.
     \end{proof}


     \subsection{A first approach}
	
	To solve PALL, we propose some heuristics that prioritize the longest routes, and thus helps to minimize $PT(s_i)$. Indeed, one can imagine that, if the messages on the longest routes are less buffered in their target than the messages using the shortest routes, the process times might be balanced.
	In an optimal solution, if $i$ is the longest route, we want $w_i =0$, such that $PT(s_i) = 2.\lambda(r_{s_i}) \geq  2.\lambda(r_{s_j}) + w_j = PT(s_j) $, where $j$ is any route $\neq i$.


     We first suggest the following heuristic:

      On the forward period, for the way forward, the messages are scheduled so that they are following each others, from the one using the longest route, to the one using the shortest route. Thus, we set the $m_{s_i}$ of the routes such that  $t(c_s,r_{s_i}) = \tau.i$, if the routes $r_{s_i}$ are ordered from the longest to the shortest.
  ...
  \begin{algorithm}[H]
    ...
    \begin{enumerate}
      ...
      \item For any route i, the deadline correspond to $m_{s_i} + T_{max}-t(c_s,r_{s_i})$, that is,  the latest date at which a message is able to quit the target switch without exceed $T_{max}$.
  ...
  \end{algorithm}
    ...
    \begin{algorithmic}
    \REQUIRE routage graph $(G,{\cal R})$, period $P$, $ T_{max}$, packet size $\tau$
   ...
   \ENDIF
	...
	We tried to look at the behavior of the that policy with different kinds of sending order for the messages.
	
	We tried the five following sending orders, each ones determines the order in which the messages will be sent from sources, and, in each case the messages are following each others in $c_s$.
	...
	\end{enumerate}
     ...
     In the rest of the section, we present the  different algorithms suggested to improve the second part of the scheduling.
      \subsection{About the periodicity}
     ...
     Once we try to schedule a message after $bp+P$, we consider that we are in the second period, and thus search a free interval in the period, large enough to put a message.
     
     This policy can significantly increase $w_i$, but ensures a periodic results. This {\bf Greedy Periodic} policy is strictly better than Greedy, because it finds some solutions that greedy would not find, by considering the periodicity in the scheduling.
     ...
     The problem PALL is very similar to a scheduling problem, but many scheduling problem does not treat about the periodic aspect. Thus, the article ``A fast algorithm for single processor scheduling'', by Barbara Simons \cite{simons1978fast} gave us an optimal algorithm to schedule some jobs which has a release time, a deadline and the same execution time. We adapted this algorithm, considering the messages as the jobs, the release times as the date at which the messages are able to come back on $c_t$, and the deadlines as the latest date at which a message have to leave $c_t$ to comeback before $m_i + T_{max}$. This algorithm gives in $\mathcal{O}(n^2\log{}n)$ a solution with the earliest  possible completion time, if it exists. Thus, it tries to minimize the time between the first and the last message in the scheduling. Let us call {\bf Minimized Scheduling (MS)} this algorithm.
    ...
    \begin{algorithmic}
    \REQUIRE A routage graph $(G,{\cal R})$,a period $P$, packet size $\tau$, $ T_{max}$, the offsets $m_i$
    ...
    This algorithm runs in $\mathcal{O}(n^3\log{}n)$. For every routes of the routage graph, the algorithm fixes the first route to comeback, and then determine the deadline of the other routes taking the minimum between the end of the period (determined by the first message that we fixed), and the deadline of the route itself(determined by $m_{s_i} + T_{max}-t(c_s,r_{s_i})$). 
    
    This algorithm finds more solutions than MS, because it compute the deadlines of the route considering the periodicity. Thus, a scheduling given by this algorithm fulfill both the periodicity and the $T_{max}$ criteria. 
    
     The goal is to choose the best starting order, such that we have a good chance to have a $(P,\tau)$ periodic assignment, even with a small period, and $T_{max}$.
   ...
   In this experiment, we fix the number of routes to 8. We want to compare the sending orders with the worst parameters, so we drew the links between $0$ and $20000$ slots. We look at the success rate of each sending order with Greedy Periodic to schedule the messages in the way back. We give to  $T_{max}$ some {\em flexibility}. This flexibility represents $T_{max} - 2\lambda(r_{longest-route})$. We let this flexibility vary from $0$ to $3000$.
...
\begin{figure}
 ...
 To evaluate the performance of our best algorithm to find solutions, that is, generating $1000$ randoms sending orders and using MSP on the way back, we compared it's performance against the actual way to manage the messages in a network. Nowadays, the networks are managed through a stochastic model. The messages in the nodes are scheduled following a FIFO rule. Even if this policy works well, in practice when the network is not too loaded, it does not ensure a minimum latency. Thus, some latency critical applications like C-RAN needs some deterministic algorithms. To model such a network, we generate some randoms offsets in the sources. Then we simulate the behavior of a stochastic network during a great number of periods. The following figure shows the average latency of a stochastic network, compared to the latency of our algorithm.
We tested the algorithms on $1000$ instances with $8$ long routes (links between $0$ and $20000$). The flexibility of the stochastic curve is computed by taking the message which have the highest process time, at which we subtract the delay of the longest route. We made a linear search to find the minimum flexibility needed by our algorithm to find a solution.
 We measure the average flexibility, for periods from $20000$ ( network fully loaded), to $50000$ (40 \% of load).
    ...
    \begin{figure}
      ...
      \end{center}
      \caption{Average latency, depending on the load of the network.}
     \end{figure}
     
    As expected, our algorithm gives some very satisfying results. There is a few cases when both the flexibility and the period are too restricted in which there is no solutions, but those case are impossible in practice. Furthermore, the stochastic policy can not ensure a minimal latency. In our context, this is crucial to use some deterministic algorithms to manage the messages in the network.
