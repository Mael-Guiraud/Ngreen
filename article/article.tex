...
\end{tikzpicture}
  ...
  Then $\{m_0',\dots,m_{n-1}'\}$ is an assignment of the same star routed network where the weights of the arcs from $s_i$ to $c_s$ are $0$
...
%       which makes the first cut more likely to apply. A last cut could be implemented: compute for every route not in $R(S)$ the set of possible positions in the backward period and verify whether at least one is contained in the useful slots of $S$.
    ...
    \subsection{Experimental evaluation}\label{sec:exp_PAZL}
      ...
      In our experiment, we generate $1000$ random instances of PAZL for $1$ to $20$ routes. 
...
\begin{figure}
     ...
     \end{figure}
      ...
      \todo{Scalability, is the exhaustive search practical for more routes: conclusion yes for our settings, no if more routes}
