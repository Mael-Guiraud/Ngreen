...
\end{tikzpicture}
  ...
  \end{center}
      ...
      In such a topology, there is only two possible collision points between the routes: $c_s$ and $c_t$. We only have to check if there is no collisions in a period at those two points.
      We consider two periods P at these vertices, the the {\bf forward period} at $c_s$, and the {\bf backward period} at $c_t$. A message issued by the source $s_1$ at time $m_{s_1}$ will reach $c_s$ at time $m_{s_1} + t(c_s,r_{s_1})$ in the forward period, and  $c_t$ at time $m_{t_1} + t(c_t,r_{t_1})$ in the backward period.
      ...
      and for any couples of routes $(r_{t_j}, r_{t_k}), j,k \in n$, we have $[t(c_t,r_{t_j})] \cap [t(c_t,r_{t_k})] = \emptyset$
%       
...
%       Let us call {\bf SN} (source node) the central node connected to leaves, and {\bf SN} (sources node), the one connected to sources. The period in those nodes are respectively called {\em forward period}, and {\em backward period}.
  ...
  In that case $MT({\cal M})$ is equal to twice the delay of the longer route, thus, $T_{max}$ is not relevant anymore. Since $w_i=0$, choosing $m_i$, the offset of the route from
  ...
  The goal is now to put messages in the forward period such that there is no collision in both periods.\\
  Here, weight of arcs $(s_i,c_s)$ can be ignored, thus, the offsets $m_{s_i}$ are calculated like if all these weight are equal to 0. To find the real values, it is easy to delay the real sending in sources by the weight of the corresponding arc.
    ...
    Send the messages in order from the shortest route to the longest route, on after each others in the forward period, without any slots between two messages, that is $m_{s_i} = \tau.i$, if the routes $r_i, i \in {0,...,n-1}$ are ordered by $\lambda(r_i)$ from the shortest to the longest one. 
...
% 	The $k$ messages uses at most $2k$ slots of size $\tau$ used in the backward period. Since $k<c$,  $2k < 3.c - k$, thus using the pigeonhole principle, there is at least one free slot in the backward period for the new message.
	...
	\ENDFOR
      ...
      \todo{ecrire l'algo? si ou le réécrire}
      ...
      \begin{enumerate}
       \item In the forward windows, each message is placed after the previous one, that is $m_{s_i} > m_{s_j}$ (here, $s_i$ is the source taken first in the algorithm, the order is not fixed). Thus, if the number of the remaining slots in the forward period is inferior than the number of slots needed to put the remaining messages, we can consider that the actual partial solution is not good.
       \item  Each time we add a new message, we cut a free interval in two new free intervals, plus the scheduled message. Each free intervals has a number of macro slots (ex: 2,6), and if the number of all macro slots is inferior than the number of remaining messages, the actual partial solution is not good.
       \item ...
      \end{enumerate}


    \subsubsection{Results}
      The following performance evaluation results compares the previous algorithms in terms of periods, and success rate for given instances. 
      In a first time, we will have a look at the results with the parameters corresponding to Cloud-RAN context, then some more global results will be shown.
      \paragraph{Short routes}
      To correspond with the C-RAN applications, we made some simulations on graphs in which $\lambda(r_{s_i}) < 2100}$, that is, at the most, 5 km between the RRH and its BBU. The messages size $\tau$ is to 2500 slots, that is, approximately 1.228 Gbps of data. The size of a slot is given by the time taken to send 64 bytes on the network, in which the links has 10Gbps throughput.
      ...
      Since we try to find a linear solution for the problem PALL, Exhaustive Generation is not a good solution. However, Greedy 3NT and Shortest-Longest are some greedy heuristics and gives us linear solutions.
      ...
      Then, we looked at the evolution of the minimal period required with the different algorithm if we significantly increase the size of the route. The following figure shows us the success rate of the algorithms, for 8 routes of size 14000 max, according to the period.
      ...
      The Exhaustive Generation curve gives us the solution when the algorithm has found a solution, and the theoretical curve gives us the number of found solutions plus the number of time the algorithm stopped. The real lower bound is probably between those two curves.
