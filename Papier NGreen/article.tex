...
%   
  ...
  One first study the latency of the C-RAN and BE traffics when the ring follows an opportunistic insertion policy: when a node wants to send some data, it uses the first free container available on the ring. If a node can not send a container, because there is no free slots or several containers must be send at the same time, the containers surplus are buffered in an insertion buffer.
  Two different method to manage this insertion buffer are proposed here. The FIFO rule is compared to another method that uses two insertion buffers: one for the containers with BE, and another for the containers with C-RAN. If there is a free container on the ring, the node wants to empty the insertion buffer with C-RAN containers first.
  Fig.~\ref{fig:resultopport} gives the cumulative distribution of both C-RAN and BE traffics latencies for the FIFO rule or the C-RAN first rule. On this experiment, the offsets of the RRH are randomly drawn in the period. The parameters of the ring are given by Fig.~\ref{fig:params}. Those results are computed over $100$ instances of optical rings that have ran during $10,000,000$ unit of time.
...
In order to ensure $0$ latency for C-RAN, the container needed to send a C-RAN message of the RRH $i$ must be free at time $m_i$. Consequently the node reserve the written access this container at time $m_i - RS$. Thus, during the last round trip of the container before $m_i$, the others nodes can only free the reserved containers.
...
\begin{prop}
 ...
 \end{prop}
 \begin{proof}
 The objective is to show that proposition by induction.
 The model is simplified in order to study the behavior of one pair, thus, both of the RRH emits some containers every slots, during $ET$ slots in the period $P$.
 {\bf Base case:} 
 Consider $ET =  \frac{P}{2}$ and two RRH each connected to the nodes $A$ and $B$.
 We assume that $A$ starts to emit a time $1$ during $ET$ slots. The containers of $A$ arrive in $B$ during the interval $[\omega(A,B);\omega(A,B)+ET[$. Then, $B$ can immediately emits its containers at time $\omega(A,B)+ET$. Those containers from $B$ arrives at $A$ at during  $ [\omega(A,B)+ET + \omega(B,A) ;ET + RS + ET] = [ET+RS; RS + P] = [ET + RS; RS] \mod P$. When $A$ tries to emit in the next period, there is a collision at the interval $[1;RS]$ in the period between the two traffics from $A$ and $B$.
Thus, for $n = 2$, it is possible to assign two RRH on the same pair if $P = 2\times ET + RS$.
 
 {\bf Induction step:}  \todo{A ecrire sans dessins}
 \end{proof}
